...
fn main() {
        ...
        .subcommands([
                         ...
                         Command::new("mrt_activity")
                             ...
                             .args([Arg::new("mrt_root")
                                 ...
                                 Arg::new("with_registry")
                                     .help("Print unused registry resources")
                                     .long("with-registry")
                                     .action(ArgAction::SetTrue),
                             ])
                     ],
    ...
    match cmd.subcommand() {
        ...
        Some(("mrt_activity", c)) => {
            ...
            let with_registry = c.get_one::<bool>("with_registry").unwrap();

            let result = modules::mrt_activity::output(base_path, mrt_root.to_owned(), max_inactive_secs.to_owned(), with_registry.to_owned());
            if result.is_err() {
                println!("{}", result.unwrap_err());
                exit(1);
            }
